...
const ControllerLogin = () => {
  ...
  const { setIsController } = useVoting();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (username === 'controller' && password === 'ctrl123') {
          ...
          <div className="rounded-3xl bg-card p-8 shadow-elevated animate-scale-in">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 rounded-xl border-2 border-border bg-muted/50 p-4 transition-all focus-within:border-primary focus-within:bg-card">
                  ...
                  <Input type="text" placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} className="border-0 bg-transparent text-lg focus-visible:ring-0" />
                </div>
            ...
            <div className="mt-6 rounded-xl bg-muted/50 p-4">
              <p className="text-center text-sm text-muted-foreground">
                <strong>Demo:</strong> username: <code className="text-primary">controller</code> / password: <code className="text-primary">ctrl123</code>
              </p>
            </div>
          </div>
        </div>
